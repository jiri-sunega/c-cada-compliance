...
## Per-release conformity test report

Every `v*` release carries `conformity-test-report.json` (machine-readable)
and `conformity-test-report.md` (human-readable), generated by
`tools/scripts/generate-conformity-report.py` from the same release job's
backend test run and added to the Ed25519-signed `release-manifest.json`'s
explicit asset list — the same signature that already covers the update
archives and SBOMs covers this test evidence too
(`c-cada-docs/architecture/2026-07-04-cra-d-annex-vii-docs-design.md §5`).
`.github/workflows/release.yml` runs the backend test suites with
per-assembly TRX evidence (compiler, runtime, security, and the four
authoring test assemblies each writing their own `.trx` file), feeds that
`test-results` directory to the generator, and attaches the resulting
`conformity-test-report.json`/`.md` pair to the signed 9-asset release
manifest alongside the product archives, SBOM/VEX documents, and the sample
bundle SBOM.
